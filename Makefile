...
# -Wall -Wextra -pedantic -std=c99 -Werror -Og -ggdb \
...
TASKIDLE=idle.tsk
#TASKA=taskTipear.tsk
TASKB=taskGameOfLifeFixedish.tsk
#TASKA=taskPong.tsk
TASKA=taskMatrix.tsk
#TASKB=taskGameOfLife.tsk
...
QEMU_ARGS = -S -drive format=raw,if=floppy,file=$(DISK_IMG) -daemonize -pidfile $(PIDFILE)
...
qemu: clean image
	@if [ -f $(PIDFILE) ]; then \
	    echo "Cerrando proceso QEMU anterior con PID: $$(cat $(PIDFILE))."; \
	    kill -9 $$(cat $(PIDFILE)) && rm -f $(PIDFILE); \
	fi
	qemu-system-i386 -s $(QEMU_ARGS)
